...
bool feasible(int k) {
	...
	// Cuenta de auxiliares para el AMO secuencial: cada polvo usado en
	// r espectáculos dentro de los k primeros añade r-1 auxiliares.
	int total_aux = 0;
	for (int d = 1; d <= P; ++d) {
		int cnt = 0;
		for (auto& p : dust_uses[d]) if (p.first < k) ++cnt;
		if (cnt > 1) total_aux += cnt - 1;
	}

	int num_vars = k + total_aux;
	TwoSAT sat(num_vars);
	...
	// Polvos: AMO secuencial con auxiliares z_0..z_{r-2} cuya lectura
	// es "alguno de los y_0..y_i está activo".
	int aux_idx = k;
	for (int d = 1; d <= P; ++d) {
		vector<int> lits;
		...
		// y_i ⇒ z_i para i = 0..r-2.
		for (int i = 0; i < r - 1; ++i) {
			sat.add_clause(neg_lit(lits[i]), var_lit(z_base + i));
		}
		// z_{i-1} ⇒ z_i para i = 1..r-2.
		for (int i = 1; i < r - 1; ++i) {
			sat.add_clause(neg_lit(var_lit(z_base + i - 1)), var_lit(z_base + i));
		}
		// z_{i-1} ⇒ ¬y_i para i = 1..r-1.
		for (int i = 1; i < r; ++i) {
			sat.add_clause(neg_lit(var_lit(z_base + i - 1)), neg_lit(lits[i]));
		}
	}
